...
class Magestore_Instantsearch_IndexController extends Mage_Core_Controller_Front_Action
{
	...
	public function loadproductAction()
    {
		...
		if($product->isSaleable())
		{
	        ...
	        $html .=" <button type='button' style='margin: -10px 0 0;' class='button' onclick='javascript:document.productAddToCartForm.submit()'><span><span>". $product_helper->__('Add to Cart') ."</span></span></button>";
			$html .="</fieldset>";
		}
	...
	public function loadproductviewAction()
    {
		...
		<div id='product_information'>
			<h3 title='".$product->getName()."'>".$product->getName()."</h3>
			<div class='product_price'>".$this->__('Price: ').$_coreHelper->currency($product->getPrice(),true,false). "</div>";
		if($product->getPrice()!=$product->getFinalPrice()){
			$html .="<div style='clear:both' class='product_price'>".$this->__('Discount: ').$_coreHelper->currency($product->getPrice()-$product->getFinalPrice(),true,false). "</div>";
		}
		if($product->isSaleable())
		{
			$html .= "<fieldset class='add-to-cart-box'>
	        ...
	        $html .=" <button type='button' class='button' style='margin: -10px 0 0;' onclick='javascript:document.productAddToCartForm.submit()'><span><span>". $product_helper->__('Add to Cart') ."</span></span></button>";
			$html .="</fieldset>";
		}
